...
浏览器 [http://localhost:5678](http://localhost:5678) → 画工作流 → Save。
...
用 `docker buildx build --platform linux/amd64 --load` 构建(客户服务器通常是 amd64)。镜像名 `n8n-<client>:1.0`。
...
```bash
REGISTRY=<你的DockerHub用户名> ./scripts/push.sh 1.0
# 例如: REGISTRY=dazey ./scripts/push.sh 1.0
# 输出: dazey/n8n-client_a:1.0
```
...
给客户 `deploy/docker-compose.yml` + 填好的 `.env`:

```bash
N8N_IMAGE=dazey/n8n-client_a:1.0
...
```bash
docker compose up -d
# 浏览器 http://<host>:5678 → 设账号密码 → 工作流自动导入
```
...
⚠️ `down -v` 会删客户在 UI 里改的工作流。工作流编辑应该回到你这边做。
...
| 场景            | 命令                                                                                                                                                                                                              |
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 起本地 n8n       | `docker run -d --name n8n-dev -p 5678:5678 -e N8N_DEFAULT_LOCALE=zh-CN -e N8N_ENTERPRISE_MOCK=true -e NODE_ENV=development -v n8n_dev_data:/home/node/.n8n --restart unless-stopped ghcr.io/deluxebear/n8n:chs` |
| 看日志           | `docker logs -f n8n-dev`                                                                                                                                                                                        |
| 新建客户分支        | `git checkout -b <user>/<client>`                                                                                                                                                                               |
| 导出工作流         | `./scripts/sync-from-running.sh`                                                                                                                                                                                |
| 构建镜像          | `./scripts/build.sh 1.0`                                                                                                                                                                                        |
| 推送镜像          | `REGISTRY=<user> ./scripts/push.sh 1.0`                                                                                                                                                                         |
